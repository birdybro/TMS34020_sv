...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 71 schema-checked entries; MOVE.RM/RM.POST/MR/MR.POST/MM semantic compatibility versus TMS34020 bus/timing ownership, RETM/RETI, MMFM/MMTM and prior deltas are quantified, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 109 primary-page-extracted encoding records/36,310 first words; MOVE.RM/RM.POST/MR/MR.POST/MM cover 5,120 ordinary indirect field words with extension/status, pointer-update and one-/two-sided timing metadata. MOVE.MR.POST same-register priority is only CORROBORATED; complete extraction remains pending |
| Architectural model slice | PROVISIONAL | 108 of 109 extracted forms have bounded successful semantics. MOVE.RM/RM.POST/MR/MR.POST/MM cover exhaustive little-endian field geometry, FE/status, pointer/alias/overlap ordering and BEN rollback; REV alone is unsupported. MOVE.MR.POST alias priority remains CORROBORATED under RSC-0036/OQ-0024. Physical bus/page/wait/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 109-entry partial decoder, serialized frontend, state leaves, clean-room field insertion/extraction/copy and address-update leaves plus prior semantic leaves, and bounded 69-operation commit path pass directed Verilator. No field-memory owner or complete processor exists |
