...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 65 schema-checked entries; RETI continuation/timing, MMFM/MMTM page-mode/timing ownership, SWAPF lock/restart, multiply flags/timing, modulus, divide, conversions and prior deltas are quantified, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 103 primary-page-extracted encoding records/31,189 first words; RETI adds normal and IX/BF frame/timing contracts while OQ-0023 retains hidden continuation layout. MMFM alignment timing, multiply and prior uncertainties plus complete extraction remain pending |
| Architectural model slice | PROVISIONAL | 102 of 103 extracted forms have bounded successful semantics over documented domains. Normal RETI restores ST/PC/SP; IX/BF roll back atomically. MMFM/MMTM and prior forms retain their bounded coverage. REV alone is unsupported. Physical page mode, waits/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 103-entry partial decoder, serialized frontend, state leaves, clean-room RETI context and MMFM/MMTM controls, divide/modulus/multiply and SWAPF leaves, graphics leaves, and bounded 69-operation commit path pass directed Verilator. RETI stack/continuation, multiple-register memory, long arithmetic, SWAPF memory/commit, and graphics forms remain noncommitting pending architectural owners. No complete processor exists |
