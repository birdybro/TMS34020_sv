...
| Architectural model slice | PROVISIONAL | All 82 currently extracted encoding forms have bounded successful semantics. CMPXY reproduces every primary row and distinguishes result-sign C/V from borrow/overflow across A/B/same-register/shared-SP cases. CLR covers every alias through canonical XOR; existing control-flow, stack, status/field, arithmetic, cache, and control-flow evidence remains unchanged. The unextracted ISA, BLMOVE continuation/overlap/timing, non-cache fault/retry, and full interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 82-entry partial decoder, serialized cache/fetch frontend, A/B/SP and masked ST storage, common semantic leaves, and a bounded fetch-to-commit path for 52 one-word, eight executable two-word, and nine executable three-word instructions pass directed Verilator and Cyclone V synthesis. CMPXY has a dedicated result-sign/equality leaf, nondestructive full-NCZV routing, cache-fed dependency coverage, and a status-only assertion; the documented one-state timing is not implemented. No architectural retirement timing or complete executable core exists |
