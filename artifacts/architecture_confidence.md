...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 64 schema-checked entries; MMFM/MMTM page-mode/timing ownership, SWAPF lock/restart, multiply flags/timing, modulus, divide, conversions and prior deltas are quantified, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 102 primary-page-extracted encoding records/31,188 first words; MMFM/MMTM add exact opposite list masks, status, logical transfers, page/dynamic-width/fault contracts, and published timing classes. MMFM alignment timing, multiply and prior uncertainties plus complete extraction remain pending |
| Architectural model slice | PROVISIONAL | 101 of 102 extracted forms have bounded successful semantics over documented domains. MMFM/MMTM cover exhaustive masks, A/B/SP, pointer/status/transfer order and timing classes; invalid lists roll back. REV alone is unsupported. Physical page mode, waits/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 102-entry partial decoder, serialized frontend, state leaves, clean-room MMFM/MMTM control, divide/modulus/multiply and SWAPF field-transform leaves, graphics leaves, and bounded 69-operation commit path pass directed Verilator. Multiple-register memory, long arithmetic, SWAPF memory/commit, and graphics forms remain noncommitting pending architectural owners. No complete processor exists |
