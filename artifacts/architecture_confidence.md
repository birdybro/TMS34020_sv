...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 62 schema-checked entries; opcode/status/timing quantification pending |
...
| ISA | PROVISIONAL | 62 primary-page-verified encoding records/19,664 first words; complete extraction pending |
| Architectural model slice | PROVISIONAL | 54 of 62 currently extracted encoding forms have bounded successful semantics, including GETPC/EXGPC sequential-PC behavior, BLMOVE non-overlap/final state and TRAPL/VLCOL success paths, with fetch through deterministic cache transaction/retry state; SLA/SLL/SRA/SRL execution, the remaining ISA, BLMOVE continuation/overlap/timing, non-cache fault/retry, and full interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 62-entry partial decoder, serialized cache/fetch frontend, A/B/SP and masked ST storage, common unary/binary/logical/rotate arithmetic, TMS34020-specific semantic leaves including exhaustive SETC pitch conversion, and a bounded fetch-to-commit path for 31 one-word, four two-word, and eight three-word instructions pass directed Verilator and Cyclone V synthesis; GETPC uses packet sequential PC and EXGPC atomically exchanges that address with the old destination before an aligned completion redirect, same-file and cross-file MOVE plus both RL forms are included, the eight decoded shift forms, SETC-pitch hidden writes, and other unsupported packets block, and no architectural retirement timing or complete executable core exists |
