...
| ISA | PROVISIONAL | 71 primary-page-verified encoding records/23,056 first words; complete extraction pending |
| Architectural model slice | PROVISIONAL | 70 of 71 currently extracted encoding forms have bounded successful semantics; newly classified EXGF remains atomic non-execution pending an independent handler. Implemented coverage includes SETF/SEXT/ZEXT across every size and both field banks with partial-status/A/B/shared-SP coverage, all 25 primary BTST inputs with RSC-0018's corrected status digit plus register hazards, every published ADDXY/SUBXY row, primary-row and register-hazard LMO coverage, SLA/SLL/SRA/SRL count/status behavior, GETPC/EXGPC sequential-PC behavior, BLMOVE non-overlap/final state and TRAPL/VLCOL success paths, with fetch through deterministic cache transaction/retry state; the unextracted ISA, BLMOVE continuation/overlap/timing, non-cache fault/retry, and full interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 71-entry partial decoder, serialized cache/fetch frontend, A/B/SP and masked ST storage, common unary/binary/logical/rotate/shift/XY/bit-test/field-extension arithmetic, TMS34020-specific semantic leaves including exhaustive SETC pitch conversion, and a bounded fetch-to-commit path for 47 one-word, four two-word, and eight three-word instructions pass directed Verilator and Cyclone V synthesis; EXGF remains blocked and noncommitting; SETF/SEXT/ZEXT cover all sizes, both field banks, partial status writes, A/B/shared-SP and dependent commits; BTST.K/R cover all 25 primary input rows plus register hazards; ADDXY/SUBXY cover all 25 primary rows plus register hazards; LMO has primary-row and register-hazard coverage; GETPC uses packet sequential PC and EXGPC atomically exchanges that address with the old destination before an aligned completion redirect; same-file and cross-file MOVE, RL, and all eight SLA/SLL/SRA/SRL forms are included; SETC-pitch hidden writes block, and no architectural retirement timing or complete executable core exists |
