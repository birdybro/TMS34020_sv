...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 106 schema-checked entries add TMS34020-only FPIXEQ/FPIXNE search/restart semantics beside CLIP/LINIT and prior deltas; the full inventory is pending |
...
| ISA | PROVISIONAL | 144 primary-page-extracted records/48,222 first words add exact TMS34020-only FPIXEQ/FPIXNE beside the prior graphics/coprocessor/memory/scalar slice; complete extraction is pending |
| Architectural model slice | PROVISIONAL | 143 of 144 extracted forms have bounded successful semantics; REV alone is unsupported. FPIX scans cover logical plane-masked reads and B10/B11/Z, but not physical grouping or continuation. Physical interfaces, unextracted ISA, and complete timing remain absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 144-entry partial decoder, serialized frontend, semantic leaves including one-step FPIX and CLIP/LINIT, and bounded 69-operation commit path pass directed Verilator. FPIX/CLIP/LINIT and CMOV expose no architectural commit without missing owners; no complete processor exists |
| Graphics/window slice | PROVISIONAL | FPIX one-step plane-masked comparisons, CLIP positive rectangles, LINIT, CPW, and four conversions are primary-verified in model/standalone RTL. Search sequencing/continuation, zero-size CLIP, complete pixel matrix, memory traces, and architectural timing remain absent |
