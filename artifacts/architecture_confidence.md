...
| ISA | PROVISIONAL | 63 primary-page-verified encoding records/20,176 first words; complete extraction pending |
| Architectural model slice | PROVISIONAL | 62 of 63 currently extracted encoding forms have bounded successful semantics; LMO is decode-only with rollback verified. Existing coverage includes SLA/SLL/SRA/SRL count/status behavior, GETPC/EXGPC sequential-PC behavior, BLMOVE non-overlap/final state and TRAPL/VLCOL success paths, with fetch through deterministic cache transaction/retry state; the unextracted ISA, LMO execution, BLMOVE continuation/overlap/timing, non-cache fault/retry, and full interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 63-entry partial decoder, serialized cache/fetch frontend, A/B/SP and masked ST storage, common unary/binary/logical/rotate/shift arithmetic, TMS34020-specific semantic leaves including exhaustive SETC pitch conversion, and a bounded fetch-to-commit path for 39 one-word, four two-word, and eight three-word instructions pass directed Verilator and Cyclone V synthesis; LMO is decode-only, GETPC uses packet sequential PC and EXGPC atomically exchanges that address with the old destination before an aligned completion redirect, same-file and cross-file MOVE, RL, and all eight SLA/SLL/SRA/SRL forms are included, SETC-pitch hidden writes and other unsupported packets block, and no architectural retirement timing or complete executable core exists |
