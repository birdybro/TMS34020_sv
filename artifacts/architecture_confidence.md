...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 72 schema-checked entries; MOVE.RM/RM.POST/MR/MR.POST/MM/MM.POST semantic compatibility versus TMS34020 bus/timing ownership, RETM/RETI, MMFM/MMTM and prior deltas are quantified, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 110 primary-page-extracted encoding records/37,334 first words; MOVE.RM/RM.POST/MR/MR.POST/MM/MM.POST cover 6,144 ordinary indirect field words with extension/status, pointer-update and one-/two-sided timing metadata. MOVE.MR.POST same-register priority and MOVE.MM.POST final shared-pointer behavior are CORROBORATED; complete extraction remains pending |
| Architectural model slice | PROVISIONAL | 109 of 110 extracted forms have bounded successful semantics. MOVE.RM/RM.POST/MR/MR.POST/MM/MM.POST cover exhaustive little-endian field geometry, FE/status, pointer/alias/overlap ordering and BEN rollback; REV alone is unsupported. MOVE.MR.POST alias priority remains CORROBORATED under RSC-0036/OQ-0024; MOVE.MM.POST's once-incremented alias destination and selected twice-incremented final pointer are CORROBORATED under RSC-0037/OQ-0025. Physical bus/page/wait/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 110-entry partial decoder, serialized frontend, state leaves, clean-room field insertion/extraction/copy, single-pointer and paired-postincrement address leaves plus prior semantic leaves, and bounded 69-operation commit path pass directed Verilator. No field-memory owner or complete processor exists |
