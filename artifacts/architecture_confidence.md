...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 67 schema-checked entries; MOVE.RM's semantic compatibility versus TMS34020 bus/timing ownership, RETM/RETI, MMFM/MMTM and prior deltas are quantified, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 105 primary-page-extracted encoding records/32,214 first words; MOVE.RM adds all 1,024 register-to-indirect field-store words and alignment/timing metadata. Complete extraction remains pending |
| Architectural model slice | PROVISIONAL | 104 of 105 extracted forms have bounded successful semantics. MOVE.RM covers exhaustive little-endian field geometry and BEN rollback; RETM/RETI and prior behavior remain. REV alone is unsupported. Physical bus/page/wait/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 105-entry partial decoder, serialized frontend, state leaves, clean-room two-word MOVE.RM insertion/classification and prior semantic leaves, plus bounded 69-operation commit path pass directed Verilator. No field-memory owner or complete processor exists |
