...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 86 schema-checked entries; MOVE field and the first three MOVB register-store semantic compatibility cases are separated from TMS34020 bus/timing ownership, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 124 primary-page-extracted encoding records/45,752 first words; seventeen MOVE RM/MR/MM families cover 13,506 field words and three register-to-memory MOVB families cover 1,056 fixed-byte words with primary addressing/status/timing metadata. MOVE.MR.POST same-register priority and MOVE.MM.POST final shared-pointer behavior are CORROBORATED; complete extraction remains pending |
| Architectural model slice | PROVISIONAL | 123 of 124 extracted forms have bounded successful semantics. MOVE field forms plus three MOVB register stores cover exhaustive little-endian field/byte geometry, status, pointer/alias/overlap/offset/absolute-word ordering and BEN rollback; REV alone is unsupported. MOVE.MR.POST and MOVE.MM.POST alias corners remain CORROBORATED. Physical bus/page/wait/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 124-entry partial decoder, serialized frontend, state leaves, clean-room fixed-byte and field insertion/extraction/copy/address leaves plus prior semantic leaves, and bounded 69-operation commit path pass directed Verilator. No field-memory owner or complete processor exists |
