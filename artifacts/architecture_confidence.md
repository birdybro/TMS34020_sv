...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 75 schema-checked entries; ordinary, postincrement and predecrement field MOVE semantic compatibility versus TMS34020 bus/timing ownership, RETM/RETI, MMFM/MMTM and prior deltas are quantified, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 113 primary-page-extracted encoding records/40,406 first words; nine ordinary/postincrement/predecrement RM/MR/MM families cover 9,216 indirect field words with extension/status, pointer-update and one-/two-sided timing metadata. MOVE.MR.POST same-register priority and MOVE.MM.POST final shared-pointer behavior are CORROBORATED; complete extraction remains pending |
| Architectural model slice | PROVISIONAL | 112 of 113 extracted forms have bounded successful semantics. The nine ordinary/postincrement/predecrement RM/MR/MM families cover exhaustive little-endian field geometry, FE/status, pointer/alias/overlap ordering and BEN rollback; REV alone is unsupported. MOVE.MR.POST alias priority remains CORROBORATED under RSC-0036/OQ-0024; MOVE.MM.POST's once-incremented alias destination and selected twice-incremented final pointer are CORROBORATED under RSC-0037/OQ-0025. Physical bus/page/wait/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 113-entry partial decoder, serialized frontend, state leaves, clean-room field insertion/extraction/copy, single-pointer and paired increment/decrement address leaves plus prior semantic leaves, and bounded 69-operation commit path pass directed Verilator. No field-memory owner or complete processor exists |
