...
| TMS34010/TMS34020 required-topic delta coverage | PROVISIONAL | 68 schema-checked entries; MOVE.RM/MR semantic compatibility versus TMS34020 bus/timing ownership, RETM/RETI, MMFM/MMTM and prior deltas are quantified, while the remaining inventory is pending |
...
| ISA | PROVISIONAL | 106 primary-page-extracted encoding records/33,238 first words; MOVE.RM/MR cover all 2,048 ordinary field-store/load words with extension/status/alignment/timing metadata. Complete extraction remains pending |
| Architectural model slice | PROVISIONAL | 105 of 106 extracted forms have bounded successful semantics. MOVE.RM/MR cover exhaustive little-endian field geometry, FE/status/alias ordering and BEN rollback; RETM/RETI and prior behavior remain. REV alone is unsupported. Physical bus/page/wait/fault/retry/16-bit behavior, continuation, unextracted ISA, and interfaces are absent |
| Synthesizable RTL slice | PROVISIONAL | Generated 106-entry partial decoder, serialized frontend, state leaves, clean-room two-word MOVE.RM insertion and MOVE.MR extraction/classification plus prior semantic leaves, and bounded 69-operation commit path pass directed Verilator. No field-memory owner or complete processor exists |
