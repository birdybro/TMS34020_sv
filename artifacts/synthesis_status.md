...
- Analysis resources: 15,185 logic cells after synthesis, 2,230 registers,
  127 pins, 0 block-memory bits, 9 DSP blocks, and 0 PLLs
- Scope: generated 148-entry partial decoder, A/B/SP register file, masked ST state,
  ...
  combinational MMFM/MMTM mask/pointer/status/timing control leaf,
  combinational RETI/RETM saved-context/result/timing classification leaf,
  combinational LINIT signed endpoint/window/line-state/status leaf,
  combinational CLIP extended-coordinate array/window intersection and status
  leaf,
  combinational one-step FPIX signed direction/effective-address/PMASK/COLOR0/
  pointer/count/terminal/status leaf,
  combinational normalized one-step DRAV COLOR1/PMASK/XY-half-add leaf,
  combinational normalized one-step FILL COLOR1/PMASK/row-column traversal leaf,
  combinational one-step FLINE decision/PATTERN/COLOR0/COLOR1/PMASK/next-state
  leaf from normalized linear increments,
  combinational CEXEC long/short command-format and state-classification leaf,
  combinational CMOVGC source/data/initial-and-reissue-command formatting leaf,
  combinational CMOVCG/CMOVCS destination/data/status/initial-and-reissue-
  command formatting leaf,
  combinational five-family CMOVCM/CMOVMC direction/count/selector/command/
  first-final-address/timing/page-break/spacer formatter,
  combinational two-long-word MOVE.RM field insertion/classification leaf,
  combinational fixed-byte MOVB register-store insertion/address-mode/timing leaf,
  combinational fixed-byte MOVB memory-load extraction/sign-extension/address-mode/timing leaf,
  combinational fixed-byte MOVB memory-copy address/byte-window/timing leaf,
  combinational two-long-word MOVE.MR field extraction/extension/classification leaf,
  combinational two-source/two-destination-long-word MOVE.MM
  copy/alignment/timing-classification leaf,
  combinational ordinary/postincrement/predecrement field-address-update leaf,
  combinational paired-postincrement effective/final field-address leaf,
  combinational paired-predecrement effective/final field-address leaf,
  combinational signed-16 field-offset effective-address leaf,
  combinational low-word/high-word absolute-address leaf,
  combinational source-offset/destination-postincrement effective/final-address
  leaf,
  decoder-controlled
...
- Result: Analysis & Synthesis successful, 0 errors, 0 warnings
- Analysis resources: 507 logic cells, 177 registers, 74 pins, 0 block-memory
  bits, 0 DSP blocks, and 0 PLLs
- Scope: generated 148-entry partial decoder, aligned instruction-start cursor,
...
- Result: Analysis & Synthesis successful, 0 errors, 0 warnings
- Analysis resources: 897 logic cells, 375 registers, 82 pins, 4,096
  block-memory bits, 0 DSP blocks, and 0 PLLs
...
- Result: Analysis & Synthesis successful, 0 errors, 0 warnings
- Analysis resources: 5,528 logic cells, 1,416 registers, 82 pins, 4,096
  block-memory bits, 0 DSP blocks, and 0 PLLs
- Memory inference: the integrated cache retains one portable 128×32 dual-port
  RAM mapped to `altsyncram`
