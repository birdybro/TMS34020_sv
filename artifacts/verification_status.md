...
| ISA database | Partial: 148 encoding records/48,738 first words | 82 `make isa-tests` cases add exact FILL.L/FILL.XY metadata and retain the complete 65,536-word unique-decode sweep; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 147 of 148 extracted forms over documented domains, cache-integrated fetch | 243 directed `make model-tests` cases add bounded FILL primary arrays, every PSIZE/lane/PMASK, rows, zero dimensions, final DADDR and rollback. REV is exact rollback. Active windows/PPOP/transparency/CST and physical sequencing remain |
| RTL semantics | Partial leaves plus bounded scalar composition | Clean-room normalized FILL leaf covers one aligned COLOR1/PMASK pixel plus row/column traversal and guards with explicit router noncommit. Existing DRAV/FLINE/FPIX/CLIP/LINIT/CEXEC/CMOV/scalar leaves remain; no complete executable sequencer exists |
...
| Graphics/video | Partial FILL, DRAV, FLINE, FPIX, CLIP, LINIT, CPW and conversion semantic leaves only | Bounded W0 FILL rows, DRAV, patterned FLINE and plane-masked FPIX model/normalized RTL, array clipping, line setup/window/status and conversion tests pass; other PPOP/transparency/window sequencing, grouping/continuation, OQ-0029, commit, pixel matrix, display/video, and full `TMS20-0024`–`TMS20-0028` remain |
