...
| ISA database | Partial: 109 encoding records/36,310 first words | 50 `make isa-tests` cases add MOVE.MR.POST pointer/timing/alias metadata and range boundaries beside MOVE.RM/RM.POST/MR/MM and prior families, plus the complete 65,536-word unique-decode sweep; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 108 of 109 extracted forms over documented domains, cache-integrated fetch | 184 directed `make model-tests` cases cover exhaustive MOVE.RM/RM.POST/MR/MR.POST/MM little-endian banks/FE/width/source/destination geometry, crossing preservation, pointer wrap, A/B/SP/alias/overlap ordering, status, timing traces, and atomic BEN rollback. MOVE.MR.POST same-register data priority is CORROBORATED under RSC-0036/OQ-0024. REV is exact rollback. Physical page/wait/fault/retry/16-bit/BEN behavior, continuation, unextracted ISA, interfaces, and full model remain |
| RTL semantics | Partial leaves plus bounded scalar composition | Clean-room field insertion, extraction/extension, copy and address-update leaves exhaust two-word little-endian geometry, pointer modes, all 25 case pairs and timing classification while the execution router prevents memory-less commit. RETI/RETM and prior leaves remain. No field-memory, return, multiple-register or complete executable sequencer exists |
...
| Memory/bus | Cache completion subset plus semantic field/stack/multiple-transfer/SWAPF boundaries | MOVE.RM/RM.POST/MR/MR.POST/MM add logical crossing stores/loads/copies, pointer updates and semantic leaves but no BEN/request/byte-strobe/RMW owner. RETI, MMFM/MMTM and SWAPF remain similarly abstract. Cache fault tests retain 1,226 accepted native reads, 36 retries, 83 faults and 43 aborts; `TMS20-0014`–`TMS20-0019` remain |
