...
| Architectural model | Partial: state/replay, bounded semantics for 74 of 75 currently extracted forms, cache-integrated fetch | 121 directed `make model-tests` cases preserve processor, memory, cache, and trace state across unsupported JUMP pending an independent handler. Existing evidence includes POPST/PUSHST ordering/timing, PUTST, EXGF, every SETF/SEXT/ZEXT size, every primary BTST row, XY/LMO/shift/register/immediate forms, bounded BLMOVE, successful TRAPL/VLCOL, cache and fetch integration. The unextracted ISA, JUMP execution, stack fault/retry/physical transfers, BLMOVE continuation/timing, interfaces, and full model remain `TMS20-0006`/`TMS20-0007` |
