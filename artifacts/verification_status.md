...
| Cache | Model plus bounded native-completion RTL leaf; full timing/pin/fault-controller path absent | 11 cache-unit plus 5 fetch-integration model tests; `make cache-tests` checks refill/lookup/LRU/controls/backpressure, all refill-index retry/fault-resume, partial abort, bypass outcomes, refill-state reset and present safety; full `TMS20-0012` remains |
| Memory/bus | Transaction-level cache completion subset only | `make fault-tests` verifies current-beat cache refill/bypass retry and bounded fault resume/abort; `TMS20-0014`–`TMS20-0019` remain |
