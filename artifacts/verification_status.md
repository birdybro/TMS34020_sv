...
| ISA database | Partial: 146 encoding records/48,736 first words | 81 `make isa-tests` cases add all 512 DRAV words/metadata and retain the complete 65,536-word unique-decode sweep; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 145 of 146 extracted forms over documented domains, cache-integrated fetch | 238 directed `make model-tests` cases add DRAV primary examples, every PSIZE/lane/PMASK, XY aliases, pitch timing, logical writes and rollback. REV is exact rollback. Window/PPOP/transparency/CST and physical sequencing remain |
| RTL semantics | Partial leaves plus bounded scalar composition | Clean-room normalized DRAV leaf covers aligned COLOR1/PMASK, independent XY half-add and guards with explicit router noncommit. Existing FLINE/FPIX/CLIP/LINIT/CEXEC/CMOV/scalar leaves remain; no complete executable sequencer exists |
...
| Graphics/video | Partial DRAV, FLINE, FPIX, CLIP, LINIT, CPW and conversion semantic leaves only | W0 replace-mode DRAV, patterned FLINE and plane-masked FPIX model/normalized RTL, array clipping, line setup/window/status and conversion tests pass; other PPOP/transparency/window sequencing, grouping/continuation, OQ-0029, commit, pixel matrix, display/video, and full `TMS20-0024`–`TMS20-0028` remain |
