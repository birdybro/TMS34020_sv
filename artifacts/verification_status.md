...
| ISA database | Partial: 52 encoding records/13,456 first words | `make isa-tests`; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, 51 encoding forms, cache-integrated fetch | 84 directed `make model-tests` cases, including complete ADDK/INC, SUBK/DEC, MOVK, MOVI, MOVE, MOVX/MOVY, RL constant/register, SETCDP/SETCMP/SETCSP, successful TRAPL/VLCOL, ADDI/SUBI/CMPI word/long forms, TI logical rows, 11 cache transaction cases and 5 fetch-integration cases; BLMOVE and the full model remain `TMS20-0007` |
