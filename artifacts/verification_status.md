...
| RTL semantics | Partial leaves plus bounded scalar composition | JUMP now reads A/B/shared-SP targets, aligns bits `[3:0]`, redirects through scalar frontend completion, and performs no register/status write. POPST/PUSHST still decode but remain explicitly rejected without writes or redirects pending memory ownership. Leaf tests cover ordinary/shared-SP JUMP commits and PUTST full-width status replacement, plus SETF/EXGF/SEXT/ZEXT in both field banks with exact partial ST masks, routing, atomic exchange, upper-register clearing, and ordered commits. Existing tests also cover all published ADDXY/SUBXY, BTST, LMO, and shift rows; all 32 SLA counts; SETC pitch conversion; and packet/write-intent boundaries. `make scalar-slice-tests` proves an EXGPC-to-GETPC-to-JUMP redirect chain and stable blocked POPST/PUSHST/BLMOVE/unclassified packets; no timed retirement or complete executable core |
