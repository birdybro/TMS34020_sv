...
| ISA database | Partial: 144 encoding records/48,222 first words | 79 `make isa-tests` cases add exact FPIXEQ/FPIXNE decode/metadata and retain the complete 65,536-word unique-decode sweep; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 143 of 144 extracted forms over documented domains, cache-integrated fetch | 227 directed `make model-tests` cases add FPIX forward/backward/exhausted/zero scans, all legal PSIZE/lane combinations, aligned PMASK/COLOR0, logical read traces, B10/B11/Z and rollback. REV is exact rollback. Physical graphics/bus continuation and complete ISA remain |
| RTL semantics | Partial leaves plus bounded scalar composition | Clean-room one-step FPIX leaf covers signed direction/effective address, all legal PSIZE/lane cases, PMASK/COLOR0 comparison, next B10/B11, terminal/Z and guards with explicit router noncommit. Existing CLIP/LINIT/CEXEC/CMOV/scalar leaves remain; no complete executable sequencer exists |
...
| Graphics/video | Partial FPIX, CLIP, LINIT, CPW and conversion semantic leaves only | Plane-masked one-step FPIX plus atomic logical model scans, array clipping, line setup/window/status and conversion tests pass; search grouping/continuation, OQ-0029, commit, pixel matrix, sequencer, display/video, and full `TMS20-0024`–`TMS20-0028` remain |
