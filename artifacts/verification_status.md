...
| Architectural model | Partial: state/replay, bounded semantics for all 80 currently extracted forms, cache-integrated fetch | 131 directed `make model-tests` cases cover every JR.L predicate/outcome, signed displacement extremes, PC wrap, exact instruction/next-PC trace fields, ST/register preservation, and two-/three-state cases while retaining all prior instruction/cache coverage. The unextracted ISA, stack fault/retry/physical transfers, BLMOVE continuation/timing, interfaces, and the full model remain `TMS20-0006`/`TMS20-0007` |
| RTL semantics | Partial leaves plus bounded scalar composition | JR.L uses an exhaustive 256-cell condition-code/NCZV truth table, is register/status neutral, performs exact signed relative redirection when true, falls through when false, and passes direct, commit, and cache-fed scalar tests including displacement/PC wrap. The previously verified DSJS, DSJ/DSJEQ/DSJNE, JUMP, and other bounded scalar operations remain executable; POPST/PUSHST remain rejected pending memory ownership. No timed retirement or complete executable core |
