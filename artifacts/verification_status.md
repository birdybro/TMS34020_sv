...
| ISA database | Partial: 108 encoding records/35,286 first words | 49 `make isa-tests` cases add MOVE.RM.POST pointer/timing metadata and range boundaries beside MOVE.RM/MR/MM and prior families, plus the complete 65,536-word unique-decode sweep; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 107 of 108 extracted forms over documented domains, cache-integrated fetch | 181 directed `make model-tests` cases cover exhaustive MOVE.RM/RM.POST/MR/MM little-endian banks/FE/width/source/destination geometry, crossing preservation, pointer wrap, A/B/SP/alias/overlap ordering, status, timing traces, and atomic BEN rollback. REV is exact rollback. Physical page/wait/fault/retry/16-bit/BEN behavior, continuation, unextracted ISA, interfaces, and full model remain |
...
| Memory/bus | Cache completion subset plus semantic field/stack/multiple-transfer/SWAPF boundaries | MOVE.RM/RM.POST/MR/MM add logical crossing stores/loads/copies, pointer updates and semantic leaves but no BEN/request/byte-strobe/RMW owner. RETI, MMFM/MMTM and SWAPF remain similarly abstract. Cache fault tests retain 1,226 accepted native reads, 36 retries, 83 faults and 43 aborts; `TMS20-0014`–`TMS20-0019` remain |
