...
| ISA database | Partial: 97 encoding records/29,588 first words | 40 `make isa-tests` cases cover all 1,024 MODS/MODU, 1,024 DIVS/DIVU, and 1,088 XY-conversion forms/metadata, plus prior fixtures and the complete 65,536-word unique-decode sweep; full decode and disclosed conflicts remain `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 96 of 97 extracted forms, cache-integrated fetch | 152 directed `make model-tests` cases cover all primary MODS/MODU rows, signed remainder, zero divisor, aliases, status masks and 35/40/3 states, while RSC-0029 discloses the unreachable published 41-state class. DIVS/DIVU and prior coverage remain; REV is exact rollback. Unextracted ISA, physical iterative timing, stack/fault behavior, BLMOVE continuation, interfaces, and full model remain `TMS20-0006`/`TMS20-0007` |
| RTL semantics | Partial leaves plus bounded scalar composition | The clean-room 32-step leaf passes divide plus signed/unsigned modulus remainder, quotient-overflow, zero-divisor, flag, and handshake tests; router guards prove no commit without architectural ownership. Prior guards/evidence remain. No divide/modulus retirement timing or complete executable core |
