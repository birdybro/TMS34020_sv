...
| Architectural model | Partial: state/replay, bounded semantics for 54 of 62 currently extracted forms, cache-integrated fetch | 91 directed `make model-tests` cases, including explicit rollback for all eight decoded-but-unsupported shift forms, GETPC/EXGPC sequential-PC/redirect behavior, complete ADDK/INC, SUBK/DEC, MOVK, MOVI, MOVE, MOVX/MOVY, RL constant/register, SETCDP/SETCMP/SETCSP, bounded BLMOVE, successful TRAPL/VLCOL, ADDI/SUBI/CMPI word/long forms, TI logical rows, 11 cache transaction cases and 5 fetch-integration cases; SLA/SLL/SRA/SRL and complete ISA execution, BLMOVE continuation/timing, interfaces, and the full model remain `TMS20-0006`/`TMS20-0007` |
