...
| Architectural model | Partial: state/replay, bounded semantics for 75 of 78 currently extracted forms, cache-integrated fetch | 123 directed `make model-tests` cases require complete DSJ/DSJEQ/DSJNE attempts to raise `UnsupportedInstruction` and atomically restore PC, registers, ST, cache, memory, and traces. Existing successful evidence covers JUMP, POPST/PUSHST, PUTST, EXGF, every SETF/SEXT/ZEXT size, every primary BTST row, XY/LMO/shift/register/immediate forms, bounded BLMOVE, TRAPL/VLCOL, cache and fetch integration. The DSJ family, unextracted ISA, stack fault/retry/physical transfers, BLMOVE continuation/timing, interfaces, and full model remain `TMS20-0006`/`TMS20-0007` |
| RTL semantics | Partial leaves plus bounded scalar composition | DSJ/DSJEQ/DSJNE classify as complete two-word packets but execution, commit, and scalar tests explicitly reject them without register, status, or PC changes. JUMP reads A/B/shared-SP targets, aligns bits `[3:0]`, redirects through scalar frontend completion, and performs no register/status write. POPST/PUSHST remain rejected pending memory ownership. Leaf tests also cover PUTST, SETF/EXGF/SEXT/ZEXT, ADDXY/SUBXY, BTST, LMO, shifts, and packet/write-intent boundaries. `make scalar-slice-tests` proves an EXGPC-to-GETPC-to-JUMP redirect chain and stable blocked DSJ/POPST/PUSHST/BLMOVE/unclassified packets; no timed retirement or complete executable core |
...
| Formal | No proof run; twenty-four runtime SVAs simulation-enabled | Four cache, four fetch, fourteen scalar, and two register-commit safety properties run under Verilator; SymbiYosys remains unavailable and `TMS20-0032` is not complete |
