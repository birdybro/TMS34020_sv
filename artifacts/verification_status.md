...
| ISA database | Partial: 44 encoding records/6,800 first words | `make isa-tests`; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, 38 encoding forms, cache-integrated fetch | 61 directed `make model-tests` cases, including ADDI/SUBI/CMPI word/long forms, TI logical rows, 11 cache transaction cases and 5 fetch-integration cases; full model remains `TMS20-0007` |
| RTL semantics | Partial leaves plus bounded scalar composition | Leaf/fetch/frontend suites cover decode, cache, packet and write-intent ordering; `make scalar-slice-tests` covers nine dependent one-word commits, complete ORI/XORI/ANDNI, dependent ADDXYI, and mixed two-/three-word ADDI/SUBI packet commits, blocked BLMOVE/CMPI.W/unclassified noncommit, and eight cache-fed dependent commits after four refill reads; no timed retirement or complete executable core |
