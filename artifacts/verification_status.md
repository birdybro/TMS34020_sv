...
| RTL semantics | Partial leaves plus bounded scalar composition | CMPXY has a dedicated warning-free leaf and nondestructive full-NCZV routing through direct, ordered-commit, bypass-fetch, and cache-refill tests, including all nine published rows, result-sign-versus-borrow boundaries, A/B/same-register/shared-SP cases, and a status-only runtime assertion. Every CLR alias and the previously verified JACC/JR.L, DSJ family, JUMP, and other bounded scalar operations retain their directed evidence; POPST/PUSHST remain rejected pending memory ownership. No timed retirement or complete executable core |
