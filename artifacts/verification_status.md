...
| Architectural model | Partial: state/replay, bounded semantics for 81 of 82 currently extracted forms, cache-integrated fetch | 135 directed `make model-tests` cases retain all prior instruction/cache coverage and require CMPXY to raise `UnsupportedInstruction` with exact state/cache rollback before implementation. The unextracted ISA, CMPXY semantics, stack fault/retry/physical transfers, BLMOVE continuation/timing, interfaces, and the full model remain `TMS20-0006`/`TMS20-0007` |
