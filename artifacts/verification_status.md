...
| Architectural model | Partial: state/replay, 32 instructions, standalone cache transaction state | 47 directed `make model-tests` cases, including all 21 register-logical and 16 immediate-logical TI example rows plus 11 cache organization/refill/retry cases; full model remains `TMS20-0007` |
...
| Cache | Transaction-level independent model tested; fetch integration/RTL/timing absent | 11 directed cache-model tests under `make model-tests`; full `TMS20-0012` remains |
