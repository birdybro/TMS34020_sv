...
| ISA database | Partial: 140 encoding records/48,218 first words | 76 `make isa-tests` cases add five CMOVCM/CMOVMC families, exact boundaries/count and pointer metadata, and retain the complete 65,536-word unique-decode sweep; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 139 of 140 extracted forms over documented domains, cache-integrated fetch | 220 directed `make model-tests` cases add CMOVCM/CMOVMC constant/register count, both directions, pre/post address order, alias/wrap, logical data traces, status preservation, and reserved/underflow/BEN rollback. REV is exact rollback. Partial physical page/spacer/wait/fault/retry/continuation and complete interfaces remain absent |
| RTL semantics | Partial leaves plus bounded scalar composition | Clean-room CEXEC and all CMOV formatters cover command/selector/count/direction/address/status/size metadata. The CMOV memory leaf proves no-command-reissue page metadata and spacer intent with explicit router noncommit; no coprocessor data sequencer/handshake/commit owner or complete executable sequencer exists |
