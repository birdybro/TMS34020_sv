...
| Architectural model | Partial: state/replay, bounded semantics for 52 of 54 currently extracted forms, cache-integrated fetch | 88 directed `make model-tests` cases, including complete ADDK/INC, SUBK/DEC, MOVK, MOVI, MOVE, MOVX/MOVY, RL constant/register, SETCDP/SETCMP/SETCSP, bounded BLMOVE, successful TRAPL/VLCOL, ADDI/SUBI/CMPI word/long forms, TI logical rows, 11 cache transaction cases and 5 fetch-integration cases; GETPC/EXGPC execution, complete ISA extraction, BLMOVE continuation/timing, interfaces, and the full model remain `TMS20-0006`/`TMS20-0007` |
