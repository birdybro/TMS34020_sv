...
| ISA database | Partial: 142 encoding records/48,220 first words | 78 `make isa-tests` cases add exact CLIP decode/metadata and retain the complete 65,536-word unique-decode sweep; full decode remains `TMS20-0006` |
| Architectural model | Partial: state/replay, bounded semantics for 141 of 142 extracted forms over documented domains, cache-integrated fetch | 223 directed `make model-tests` cases add CLIP positive inside/partial/outside/overflow/max-dimension results, B2/B7/Z/V effects, no-data trace, and zero/invalid rollback. REV is exact rollback. OQ-0029, complete graphics/bus interfaces, continuation and complete ISA remain |
| RTL semantics | Partial leaves plus bounded scalar composition | Clean-room CLIP leaf passes eight semantic/guard rows and warning-free lint with explicit router noncommit pending four implied reads and atomic B2/B7/status retirement. Existing LINIT/CEXEC/CMOV and scalar leaves remain; no complete executable sequencer exists |
...
| Graphics/video | Partial CLIP, LINIT, CPW and XY-conversion semantic leaves only | Positive-dimension extended-coordinate array clipping, signed line setup/window/status, CPW outcode, and conversion equation/pitch-class tests pass; OQ-0029, architectural commit, pixel matrix, sequencer, memory traces, display/video, and full `TMS20-0024`–`TMS20-0028` remain |
