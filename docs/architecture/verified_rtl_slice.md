...
|---|---|---|
| `rtl/core/tms34020_decode.sv` | Classification and instruction length for the 83 entries currently present in the canonical ISA database, including all 32 REV destinations; all other first words remain explicitly unclassified | TI *TMS34020 User's Guide*, August 1990, individual instruction pages listed in `docs/generated/tms34020_isa.yaml` |
...
REV is classified but intentionally absent from the execution lists above.
The leaf test drives `0020h` through `tms34020_register_execute` and requires
zero support, register-write, and status-write intents. This prevents a shared
TMS34010 constant or the pinned MAME defect in RSC-0021 from becoming
architectural RTL until an explicit device profile supplies the complete
revision word.
...
so its 8,777 logic-cell/2,048-register resource count is not a core-area
...
packet assembler and generated decoder. Its observability wrapper uses 419
...
composition with zero errors/warnings to 5,330 logic cells, 1,414 registers,
