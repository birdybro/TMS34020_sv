...
|---|---|---|
| `rtl/core/tms34020_decode.sv` | Classification and instruction length for the 44 entries currently present in the canonical ISA database; all other first words remain explicitly unclassified | TI *TMS34020 User's Guide*, August 1990, individual instruction pages listed in `docs/generated/tms34020_isa.yaml` |
...
so its 6,028 logic-cell/2,021-register resource count is not a core-area
...
composition with zero errors/warnings to 3,792 logic cells, 1,357 registers,
...
register/status operations. Complete CMPI.W packets are explicitly tested as
decoded, blocked, and state-stable. There is no timing sequencer,
