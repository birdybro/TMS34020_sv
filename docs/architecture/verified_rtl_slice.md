...
valid, and Z/V result of the standalone CLIP leaf. The register-execution
router explicitly rejects LINIT and CLIP because neither multi-register atomic
architectural owner exists. The wrapper deliberately
retains both the original raw state leaves and the integrated commit instance,
so its 14,025 logic-cell/2,230-register/9-DSP resource count is not a core-area
...
packet assembler and generated decoder. Its observability wrapper uses 498
...
zero errors/warnings to 885 logic cells, 375 registers, and 4,096 block-memory
...
composition with zero errors/warnings to 5,509 logic cells, 1,416 registers,
