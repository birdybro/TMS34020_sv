...
|---|---|---|
| `rtl/core/tms34020_decode.sv` | Classification and instruction length for the 42 entries currently present in the canonical ISA database; all other first words remain explicitly unclassified | TI *TMS34020 User's Guide*, August 1990, individual instruction pages listed in `docs/generated/tms34020_isa.yaml` |
...
| `rtl/core/tms34020_register_commit.sv` | Externally gated, single-edge register/ST state commit for 23 one-word operations, two-word ADDI.W, and complete three-word ANDNI/ORI/XORI/ADDXYI/ADDI.L packets; unsupported or length-mismatched packets cannot mutate state | TI *TMS34020 User's Guide*, August 1990, §4.1 and the individual instruction pages cited for `tms34020_register_execute` |
| `rtl/core/tms34020_scalar_slice.sv` | Conservative cache/fetch-to-register composition for 29 verified scalar operations; decoded unsupported and unclassified packets remain stable and noncommitting | TI *TMS34020 User's Guide*, August 1990, §4.1 and the individual instruction pages cited for `tms34020_register_execute` |
...
- twenty-eight ordered commit checks covering EINT, SETC, GETST, INC, DINT, DEC,
  ...
  through the shared SP alias. These checks prove that a later operation
...
two dependent ADDXYI packet commits, dependent ADDI.W/ADDI.L/ADDI.W packet
commits, unclassified-word noncommit, and a cache-enabled pass that feeds eight
...
The same scalar regression fetches a complete two-word SUBI.W packet and proves
that it remains decoded but unsupported, blocked, and state-stable. SUBI model
coverage is not an RTL execution claim.
...
so its 5,859 logic-cell/2,021-register resource count is not a core-area
...
packet assembler and generated decoder. Its observability wrapper uses 350
...
zero errors/warnings to 723 logic cells, 372 registers, and 4,096 block-memory
...
composition with zero errors/warnings to 3,679 logic cells, 1,357 registers,
and 4,096 block-memory bits. The observability wrapper is not a core-area
estimate, and no fit or TimeQuest result exists.

## Explicitly absent

There is a bounded serialized opcode-to-register execution path for only 29
register/status operations, but no timing sequencer, processor-derived
retirement boundary, interrupt logic, complete memory access, page mode,
