...
|---|---|---|
| `rtl/core/tms34020_decode.sv` | Classification and instruction length for the 11 entries currently present in the canonical ISA database; all other first words remain explicitly unclassified | TI *TMS34020 User's Guide*, August 1990, individual instruction pages listed in `docs/generated/tms34020_isa.yaml` |
...
keeps both register-file read ports, arithmetic flags, decoder outputs, and
RPIX timing outputs observable. This is an early portability check only:
...
interface, display subsystem, original-pin bus, or game wrapper. ADDXYI and
RPIX are combinational semantic leaves; their presence does not mean either
instruction can execute in RTL.
