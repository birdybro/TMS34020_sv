...
|---|---|---|
| `rtl/core/tms34020_decode.sv` | Classification and instruction length for the 85 entries currently present in the canonical ISA database, including all 32 REV destinations, 32 TRAP vectors, and 32 RETS argument counts; all other first words remain explicitly unclassified | TI *TMS34020 User's Guide*, August 1990, individual instruction pages listed in `docs/generated/tms34020_isa.yaml` |
...
REV, TRAP, and RETS are classified but intentionally absent from the execution
lists above. Leaf tests drive `0020h`, `0900h`, `091Fh`, `0960h`, and `097Fh` through
...
so its 8,792 logic-cell/2,048-register resource count is not a core-area
...
zero errors/warnings to 788 logic cells, 373 registers, and 4,096 block-memory
...
composition with zero errors/warnings to 5,286 logic cells, 1,414 registers,
