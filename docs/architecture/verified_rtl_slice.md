...
|---|---|---|
| `rtl/core/tms34020_decode.sv` | Classification and instruction length for the 65 entries currently present in the canonical ISA database; all other first words remain explicitly unclassified | TI *TMS34020 User's Guide*, August 1990, individual instruction pages listed in `docs/generated/tms34020_isa.yaml` |
...
- ADDXY and SUBXY base/end decode boundaries plus explicit rejection at both
  register-execution and atomic-commit owners until their semantic RTL exists;
...
- ordered commit checks covering EINT, SETC, GETST, ADDK/INC, DINT,
  ...
  source commit, unsupported ADDXY/SUBXY/BLMOVE rejection, state-neutral NOP,
  AND, OR, XOR,
...
GETST, from exactly four refill long-word reads. Nine runtime assertions across
...
so its 7,695 logic-cell/2,021-register resource count is not a core-area
...
packet assembler and generated decoder. Its observability wrapper uses 380
...
zero errors/warnings to 755 logic cells, 373 registers, and 4,096 block-memory
...
composition with zero errors/warnings to 4,559 logic cells, 1,387 registers,
and 4,096 block-memory bits. The observability wrapper is not a core-area
estimate, and no fit or TimeQuest result exists.

## Explicitly absent

There is a bounded serialized opcode-to-register execution path for only 52
