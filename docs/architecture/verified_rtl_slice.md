...
|---|---|---|
| `rtl/core/tms34020_decode.sv` | Classification and instruction length for the 67 entries currently present in the canonical ISA database; all other first words remain explicitly unclassified | TI *TMS34020 User's Guide*, August 1990, individual instruction pages listed in `docs/generated/tms34020_isa.yaml` |
...
- ordered commit checks covering EINT, SETC, GETST, ADDK/INC, DINT,
  ...
  checks also include dependent ADDXY/SUBXY register commits and shared-SP
  source/destination commits. They prove that
...
noncommit for one-word BLMOVE, both decode-only BTST forms, and unclassified
packets, complete
...
shift result, GETPC sequential-PC capture,
...
GETST, from exactly four refill long-word reads. Ten runtime assertions across
the scalar and commit
owners constrain acceptance, blocked writes, single-pulse commit, mutually
exclusive execution ownership, atomic LMO/shift/XY writes, and aligned
...
so its 7,986 logic-cell/2,021-register resource count is not a core-area
...
packet assembler and generated decoder. Its observability wrapper uses 393
...
zero errors/warnings to 765 logic cells, 373 registers, and 4,096 block-memory
...
composition with zero errors/warnings to 4,806 logic cells, 1,387 registers,
and 4,096 block-memory bits. The observability wrapper is not a core-area
estimate, and no fit or TimeQuest result exists.

## Explicitly absent

There is a bounded serialized opcode-to-register execution path for only 54
...
composition. BTST.K and BTST.R have verified generated-decode boundaries but
are deliberately rejected by register execution and atomic commit pending an
