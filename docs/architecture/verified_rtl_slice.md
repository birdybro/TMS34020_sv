...
| `rtl/core/tms34020_register_commit.sv` | Externally gated, single-edge register/ST state commit for 23 one-word operations and complete three-word ANDNI/ORI/XORI packets; unsupported or length-mismatched packets cannot mutate state | TI *TMS34020 User's Guide*, August 1990, §4.1 and the individual instruction pages cited for `tms34020_register_execute` |
| `rtl/core/tms34020_scalar_slice.sv` | Conservative cache/fetch-to-register composition for 26 verified scalar operations; decoded unsupported and unclassified packets remain stable and noncommitting | TI *TMS34020 User's Guide*, August 1990, §4.1 and the individual instruction pages cited for `tms34020_register_execute` |
...
- twenty-one ordered commit checks covering EINT, SETC, GETST, INC, DINT, DEC,
  ...
  and complete ORI/XORI/ANDNI packets. These checks prove that a later
  operation observes the preceding committed register/ST state; they do not
  assign an architectural cycle count to the commit edge.
...
so its 5,583 logic-cell/2,021-register resource count is not a core-area
...
composition with zero errors/warnings to 3,512 logic cells, 1,357 registers,
and 4,096 block-memory bits. The observability wrapper is not a core-area
estimate, and no fit or TimeQuest result exists.

## Explicitly absent

There is a bounded serialized opcode-to-register execution path for only 26
