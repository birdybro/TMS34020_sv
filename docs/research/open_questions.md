...
| OQ-0013 | How should generated decode encode short `JRcc` while reserving `Ccode00h` for long `JRcc` and `Ccode80h` for `JAcc`, and does another TI source state the legal hardware domain more explicitly than the assembler's excluded offsets? | Collision-free short/long/absolute conditional-jump decode, exhaustive opcode classification, and illegal-word policy | Other guide revisions, TI assembler diagnostics/listings, silicon tests, or TI decode patents |
