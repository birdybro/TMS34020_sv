...
class ExecutionTests(unittest.TestCase):
    ...
    def test_drav_primary_replace_examples_and_timing(self) -> None:
        for pixel_size, destination, expected_destination, expected_word in (
            (1, 0x0040_0040, 0x0050_0050, 0x8889),
            (2, 0x0040_0020, 0x0050_0030, 0x888B),
            (4, 0x0040_0010, 0x0050_0020, 0x888F),
            (8, 0x0040_0008, 0x0050_0018, 0x88FF),
            (16, 0x0040_0004, 0x0050_0014, 0xFFFF),
        ):
            with self.subTest(pixel_size=pixel_size):
                model = Tms34020Model()
                model.load_program([0xF620])
                model.state.write_io(PSIZE_ADDRESS, pixel_size)
                model.state.write_io(CONVDP_ADDRESS, 0x0016)
                model.state.write_reg("A", 0, destination)
                model.state.write_reg("A", 1, 0x0010_0010)
                model.state.write_reg("B", 3, 0x200)
                model.state.write_reg("B", 4, 0x0001_0000)
                model.state.write_reg("B", 9, 0xFFFF_FFFF)
                model.state.memory.write_bits(0x0001_8040, 16, 0x8888)
                ...
                self.assertEqual(write["purpose"], "drav_replace")
                self.assertEqual(write["bit_address"], 0x0001_8040)
                self.assertEqual(
                    model.state.memory.read_bits(0x0001_8040, 16),
                    expected_word,
                )
                self.assertEqual(
                    model.state.read_reg("A", 0), expected_destination
                )
                self.assertEqual(model.state.st, 0xDABC_DEF0)
                self.assertEqual(event.machine_states, 4)

    def test_drav_xy_half_add_files_alias_and_shared_sp(self) -> None:
        for opcode, register_file, index, initial, increment, expected in (
            (0xF630, "B", 0, 0x0001_0001, 0x0001_FFFF, 0x0002_0000),
            (0xF642, "A", 2, 0x0001_0001, 0x0001_0001, 0x0002_0002),
            (0xF7FF, "A", 15, 0x8000_8000, 0x8000_8000, 0x0000_0000),
        ):
            with self.subTest(opcode=f"{opcode:04X}"):
                model = Tms34020Model()
                model.load_program([opcode])
                model.state.write_io(PSIZE_ADDRESS, 1)
                model.state.write_io(CONVDP_ADDRESS, 0x001A)
                model.state.write_reg(register_file, index, initial)
                source_index = (opcode >> 5) & 0xF
                model.state.write_reg(register_file, source_index, increment)
                ...
                self.assertEqual(
                    model.state.read_reg(register_file, index), expected
                )
                if index == 15:
                    self.assertEqual(model.state.read_reg("B", 15), expected)

        model = Tms34020Model()
        model.load_program([0xF639])
        model.state.write_io(PSIZE_ADDRESS, 8)
        model.state.write_io(CONVDP_ADDRESS, 0x001A)
        model.state.write_reg("B", 1, 0x0001_0001)
        model.state.write_reg("B", 9, 0xA5A5_A5A5)
        event = model.step()
        write = next(
            item for item in event.transactions
            if item["class"] == "pixel_write"
        )
        self.assertEqual(write["source_value"], 0xA5)
        self.assertEqual(model.state.read_reg("B", 9), 0xA5A6_A5A6)

    def test_drav_all_pixel_sizes_lanes_and_plane_mask(self) -> None:
        for pixel_size in (1, 2, 4, 8, 16, 32):
            pixel_mask = (
                0xFFFF_FFFF
                if pixel_size == 32
            ...
            source = 0xA5A5_5A5B & pixel_mask
            raw = 0x5A5A_A5A4 & pixel_mask
            protected = 0xAAAA_AAAA & pixel_mask
            expected = (raw & protected) | (source & ~protected & pixel_mask)
            for lane in range(0, 32, pixel_size):
                with self.subTest(pixel_size=pixel_size, lane=lane):
                    address = 0x400 + lane
                    model = Tms34020Model()
                    ...
                    model.state.write_io(
                        PMASKL_ADDRESS, (protected << lane) & 0xFFFF
                    )
                    model.state.write_io(
                        PMASKH_ADDRESS, (protected << lane) >> 16
                    )
                    model.state.write_reg("A", 0, 0)
                    model.state.write_reg("A", 1, 0)
                    model.state.write_reg("B", 4, address)
                    model.state.write_reg("B", 9, source << lane)
                    model.state.memory.write_bits(address, pixel_size, raw)
                    ...
                    self.assertEqual(
                        model.state.memory.read_bits(address, pixel_size),
                        expected,
                    )
                    self.assertEqual(event.machine_states, 4)

    def test_drav_all_pitch_classes_and_timing(self) -> None:
        for conversion, pitch, expected_address, expected_states, pitch_class in (
            (0x001A, 0x100, 0x20, 4, "power_of_two"),
            (0x1B1A, 0x100, 0x30, 5, "two_powers_of_two"),
            (0x0000, 0x100, 0x100, 16, "arbitrary"),
        ):
            with self.subTest(pitch_class=pitch_class):
                ...
                model.state.write_reg("A", 0, 0x0001_0000)
                model.state.write_reg("A", 1, 0)
                ...
                self.assertEqual(write["bit_address"], expected_address)
                self.assertEqual(event.machine_states, expected_states)
                self.assertIn(
    ...
    def test_drav_unsupported_modes_and_alignment_roll_back(self) -> None:
        for config, dpyctl, control, pixel_size, offset, error_type in (
            (1, 0, 0, 4, 0x100, UnsupportedInstruction),
            (0, 1 << 11, 0, 4, 0x100, UnsupportedInstruction),
            (0, 0, 1 << 10, 4, 0x100, UnsupportedInstruction),
            (0, 0, 1 << 5, 4, 0x100, UnsupportedInstruction),
            (0, 0, 1 << 6, 4, 0x100, UnsupportedInstruction),
            (0, 0, 0, 3, 0x100, ModelError),
            (0, 0, 0, 4, 0x102, ModelError),
        ):
            with self.subTest(
                config=config,
                dpyctl=dpyctl,
    ...
    def test_fill_l_primary_replace_example_and_final_daddr(self) -> None:
        initial_words = (
            0x1100, 0x3322, 0x5544, 0x7766,
            0x9988, 0xBBAA, 0xDDCC, 0xFFEE,
        )
        expected_words = (
            0x1100, 0x3030, 0x3030, 0x3030,
            0x3030, 0x3030, 0x3030, 0xFF30,
        )
        model = Tms34020Model()
        model.load_program([0x0FC0])
        model.state.write_io(PSIZE_ADDRESS, 8)
        model.state.write_reg("B", 2, 0x0000_2010)
        model.state.write_reg("B", 3, 0x0000_0080)
        model.state.write_reg("B", 7, 0x0002_000D)
        model.state.write_reg("B", 9, 0x3030_3030)
        model.state.st = 0xDABC_DEF0
        for row_start in (0x2000, 0x2080):
            for index, value in enumerate(initial_words):
                model.state.memory.write_bits(
                    row_start + index * 16, 16, value
                )
        ...
        self.assertEqual(writes[0]["bit_address"], 0x2010)
        self.assertEqual(writes[-1]["bit_address"], 0x20F0)
        for row_start in (0x2000, 0x2080):
            self.assertEqual(
                tuple(
                    model.state.memory.read_bits(
                        row_start + index * 16, 16
                    )
                    for index in range(8)
                ),
                expected_words,
            )
        self.assertEqual(model.state.read_reg("B", 2), 0x0000_2110)
        self.assertEqual(model.state.read_reg("B", 7), 0x0002_000D)
        self.assertEqual(model.state.st, 0xDABC_DEF0)

    def test_fill_xy_primary_replace_example_and_linear_final_daddr(self) -> None:
        initial_words = (
            0x3210, 0x7654, 0xBA98, 0xFEDC,
            0x3210, 0x7654, 0xBA98, 0xFEDC,
        )
        expected_words = (
            0x3210, 0xF654, 0xFFFF, 0xFFFF,
            0xFFFF, 0xFFFF, 0xBA9F, 0xFEDC,
        )
        model = Tms34020Model()
        model.load_program([0x0FE0])
        model.state.write_io(PSIZE_ADDRESS, 4)
        model.state.write_io(CONVDP_ADDRESS, 0x0017)
        model.state.write_reg("B", 2, 0x0052_0007)
        model.state.write_reg("B", 3, 0x0000_0100)
        model.state.write_reg("B", 4, 0x0001_0000)
        model.state.write_reg("B", 7, 0x0003_0012)
        model.state.write_reg("B", 9, 0xFFFF_FFFF)
        model.state.st = 0x5ABC_DEF0
        for row_start in (0x15200, 0x15300, 0x15400):
            for index, value in enumerate(initial_words):
                model.state.memory.write_bits(
                    row_start + index * 16, 16, value
                )
        ...
        self.assertEqual(len(writes), 54)
        self.assertEqual(writes[0]["bit_address"], 0x1521C)
        self.assertEqual(writes[-1]["bit_address"], 0x15460)
        for row_start in (0x15200, 0x15300, 0x15400):
            self.assertEqual(
                tuple(
                    model.state.memory.read_bits(
        ...
        self.assertEqual(model.state.read_reg("B", 2), 0x0001_551C)
        self.assertEqual(model.state.read_reg("B", 7), 0x0003_0012)
        self.assertEqual(model.state.st, 0x5ABC_DEF0)
        self.assertIn(
            "XY conversion pitch class: power_of_two", event.notes
        )

    def test_fill_all_pixel_sizes_lanes_masks_and_both_forms(self) -> None:
        for opcode in (0x0FC0, 0x0FE0):
            for pixel_size in (1, 2, 4, 8, 16, 32):
                pixel_mask = (
                    0xFFFF_FFFF
                    if pixel_size == 32
                ...
                for lane in range(0, 32, pixel_size):
                    with self.subTest(
                        ...
                        model.state.write_reg(
                            "B", 2, address if opcode == 0x0FC0 else 0
                        )
                        model.state.write_reg("B", 3, 0x20)
                        model.state.write_reg(
                            "B", 4, address if opcode == 0x0FE0 else 0
                        )
                        ...
                        self.assertEqual(write["bit_address"], address)
                        self.assertEqual(write["source_value"], source)
                        self.assertEqual(write["plane_mask"], protected)
                        self.assertEqual(write["value"], expected)
                        self.assertEqual(
    ...
    def test_fill_row_traversal_zero_dimensions_and_linear_w_ignore(self) -> None:
        model = Tms34020Model()
        model.load_program([0x0FC0])
        model.state.write_io(PSIZE_ADDRESS, 8)
        model.state.write_io(CONTROL_ADDRESS, 3 << 6)
        model.state.write_reg("B", 2, 0x108)
        ...
        self.assertEqual(
            [
                item["bit_address"] for item in event.transactions
                if item["class"] == "pixel_write"
            ],
        ...
        for dimensions in (0x0000_0003, 0x0002_0000):
            with self.subTest(dimensions=f"{dimensions:08X}"):
                empty = Tms34020Model()
                empty.load_program([0x0FC0])
                empty.state.write_io(PSIZE_ADDRESS, 8)
                empty.state.write_reg("B", 2, 0x200)
                ...
                self.assertEqual(empty.state.read_reg("B", 2), 0x200)

    def test_fill_unsupported_modes_bounds_and_alignment_roll_back(self) -> None:
        for (
            opcode, config, dpyctl, control, pixel_size,
            daddr, dptch, dimensions, error_type,
        ) in (
            (0x0FC0, 1, 0, 0, 4, 0x100, 0x20, 0x0001_0001,
             UnsupportedInstruction),
            (0x0FC0, 0, 1 << 11, 0, 4, 0x100, 0x20, 0x0001_0001,
             UnsupportedInstruction),
            (0x0FC0, 0, 0, 1 << 10, 4, 0x100, 0x20, 0x0001_0001,
             UnsupportedInstruction),
            (0x0FC0, 0, 0, 1 << 5, 4, 0x100, 0x20, 0x0001_0001,
             UnsupportedInstruction),
            (0x0FE0, 0, 0, 1 << 6, 4, 0, 0x20, 0x0001_0001,
             UnsupportedInstruction),
            (0x0FC0, 0, 0, 0, 3, 0x100, 0x20, 0x0001_0001,
             ModelError),
            (0x0FC0, 0, 0, 0, 4, 0x102, 0x20, 0x0001_0001,
             ModelError),
            (0x0FC0, 0, 0, 0, 4, 0x100, 0x22, 0x0001_0001,
             ModelError),
            (0x0FC0, 0, 0, 0, 4, 0x100, 0x20, 0x0002_FFFF,
             UnsupportedInstruction),
        ):
            with self.subTest(
                opcode=f"{opcode:04X}",
    ...
    def test_fline_horizontal_pattern_draw_and_primary_timing(self) -> None:
        ...
        model.state.write_io(PSIZE_ADDRESS, 4)
        model.state.write_io(CONVDP_ADDRESS, 0x001A)
        model.state.write_reg("B", 0, 0xFFFF_FFFE)
        model.state.write_reg("B", 2, 0x100)
        model.state.write_reg("B", 3, 0x20)
        model.state.write_reg("B", 7, 0x0000_0002)
        model.state.write_reg("B", 8, 0x5555_5555)
        model.state.write_reg("B", 9, 0xAAAA_AAAA)
        model.state.write_reg("B", 10, 3)
        model.state.write_reg("B", 11, 1)
        model.state.write_reg("B", 12, 1)
        model.state.write_reg("B", 13, 0x0000_0005)
        model.state.st = 0xDABC_DEF0
        ...
        self.assertEqual(
            [(item["bit_address"], item["value"]) for item in writes],
            [(0x100, 0xA), (0x104, 0x5), (0x108, 0xA)],
        )
        self.assertEqual(model.state.memory.read_bits(0x100, 12), 0xA5A)
        self.assertEqual(model.state.read_reg("B", 0), 0xFFFF_FFFE)
        self.assertEqual(model.state.read_reg("B", 2), 0x10C)
        self.assertEqual(model.state.read_reg("B", 10), 0)
        self.assertEqual(model.state.read_reg("B", 13), 0xA000_0000)
        self.assertEqual(model.state.st, 0xDABC_DEF0)
        self.assertEqual(event.machine_states, 24)

    def test_fline_algorithm_boundary_and_arbitrary_pitch(self) -> None:
        for opcode, expected_address, expected_decision in (
            (0xDE1A, 0x308, 0xFFFF_FFFC),
            (0xDE9A, 0x208, 0x0000_0004),
        ):
            with self.subTest(opcode=f"{opcode:04X}"):
                model = Tms34020Model()
                model.load_program([opcode])
                ...
                model.state.write_reg("B", 0, 0)
                model.state.write_reg("B", 2, 0x200)
                model.state.write_reg("B", 3, 0x100)
                model.state.write_reg("B", 7, 0x0002_0004)
                model.state.write_reg("B", 9, 0xA5A5_A5A5)
                model.state.write_reg("B", 10, 1)
                model.state.write_reg("B", 11, 0x0001_0001)
                model.state.write_reg("B", 12, 0x0000_0001)
                model.state.write_reg("B", 13, 1)
                ...
                self.assertEqual(write["value"], 0xA5)
                self.assertEqual(write["diagonal"], opcode == 0xDE1A)
                self.assertEqual(
                    model.state.read_reg("B", 2), expected_address
                )
                self.assertEqual(
                    model.state.read_reg("B", 0), expected_decision
                )
                self.assertEqual(model.state.read_reg("B", 10), 0)
                self.assertEqual(model.state.read_reg("B", 13), 0x8000_0000)
                self.assertEqual(event.machine_states, 53)

    def test_fline_all_pixel_sizes_lanes_and_plane_mask(self) -> None:
        for pixel_size in (1, 2, 4, 8, 16, 32):
            pixel_mask = (
                0xFFFF_FFFF
                if pixel_size == 32
            ...
            for lane in range(0, 32, pixel_size):
                with self.subTest(pixel_size=pixel_size, lane=lane):
                    ...
                    model.state.write_io(CONVDP_ADDRESS, 0x001A)
                    model.state.write_reg("B", 0, 0xFFFF_FFFF)
                    model.state.write_reg("B", 2, address)
                    model.state.write_reg("B", 7, 0)
                    model.state.write_reg("B", 9, source << lane)
                    model.state.write_reg("B", 10, 1)
                    model.state.write_reg("B", 13, 1)
                    model.state.memory.write_bits(
                    ...
                    self.assertEqual(
                        event.machine_states,
                        18 if pixel_size <= 4 else 17,
                    )

    def test_fline_two_power_pitch_formula_and_increment(self) -> None:
        model = Tms34020Model()
        model.load_program([0xDE1A])
        model.state.write_io(PSIZE_ADDRESS, 8)
        model.state.write_io(CONVDP_ADDRESS, 0x1B1A)
        model.state.write_reg("B", 0, 0xFFFF_FFFF)
        model.state.write_reg("B", 2, 0x500)
        model.state.write_reg("B", 9, 0xA5A5_A5A5)
        model.state.write_reg("B", 10, 1)
        model.state.write_reg("B", 12, 0x0001_0000)
        model.state.write_reg("B", 13, 1)

        event = model.step()

        self.assertEqual(model.state.read_reg("B", 2), 0x530)
        self.assertEqual(event.machine_states, 20)
        self.assertIn(
            "XY conversion pitch class: two_powers_of_two",
            event.notes,
        )

    def test_fline_zero_and_negative_count_are_no_op_draws(self) -> None:
        for count in (0, 0xFFFF_FFFF):
            with self.subTest(count=f"{count:08X}"):
                model = Tms34020Model()
                model.load_program([0xDE1A])
                model.state.write_io(PSIZE_ADDRESS, 4)
                model.state.write_io(CONVDP_ADDRESS, 0x001A)
                model.state.write_reg("B", 0, 0x1234_5678)
                model.state.write_reg("B", 2, 0x100)
                model.state.write_reg("B", 10, count)
                model.state.write_reg("B", 13, 0x89AB_CDEF)
                ...
                self.assertEqual(model.state.read_reg("B", 0), 0x1234_5678)
                self.assertEqual(model.state.read_reg("B", 2), 0x100)
                self.assertEqual(model.state.read_reg("B", 10), count)
                self.assertEqual(model.state.read_reg("B", 13), 0x89AB_CDEF)
                self.assertEqual(event.machine_states, 15)

    def test_fline_unsupported_modes_and_alignment_roll_back(self) -> None:
        for config, dpyctl, control, pixel_size, address, error_type in (
            (1, 0, 0, 4, 0x100, UnsupportedInstruction),
            (0, 1 << 11, 0, 4, 0x100, UnsupportedInstruction),
            (0, 0, 1 << 10, 4, 0x100, UnsupportedInstruction),
            (0, 0, 1 << 5, 4, 0x100, UnsupportedInstruction),
